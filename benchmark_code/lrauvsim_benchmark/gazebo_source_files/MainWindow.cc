  ...
  : dataPtr(new MainWindowPrivate)
{
  ...
  // -----------------------------------------------------------------------
  // Setup FPS logging to CSV
  static std::ofstream guiFpsFile("render_fps.csv");
  guiFpsFile << "time,fps" << std::endl;  // Optional: write header once

  static auto lastTime = std::chrono::steady_clock::now();

  // Connect to frameSwapped signal to measure GUI FPS
  QObject::connect(this->dataPtr->quickWindow, &QQuickWindow::frameSwapped,
    [=]() mutable
    {
      ...
      if (dt > 0)
      {
        ...
        double wallTime = std::chrono::duration<double>(
            now.time_since_epoch()).count();

        guiFpsFile << wallTime << "," << fps << std::endl;
      }
    });
  //-------------------------------------------------------------------------
