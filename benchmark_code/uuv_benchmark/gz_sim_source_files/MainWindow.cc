...
MainWindow::~MainWindow()
{
  ...
  // Close the file on shutdown (for FPS .csv)
  if (this->fpsLogFile.is_open())
    this->fpsLogFile.close();
}
...
void MainWindow::Init()
{
  ...
  //--------------------------------------------------------
  // Start FPS logging
  this->fpsLogFile.open("render_fps.csv");
  this->fpsLogFile << "WallTime(s),FPS" << std::endl;

  this->fpsTimer = new QTimer(this);
  connect(this->fpsTimer, &QTimer::timeout, this, &MainWindow::LogFPS);
  this->fpsTimer->start(500);  // Log every 0.5 seconds
}

/////////////////////////////////////////////////


void MainWindow::LogFPS()
{
  auto cam = gui::get_active_camera();
  if (!cam)
    return;

  double fps = cam->AvgFPS();  // Average rendering FPS
  double wallTime = gazebo::common::Time::GetWallTime().Double();  // Timestamp

  //std::cout << "[FPS] " << fps << " at " << wallTime << "s" << std::endl;

  if (this->fpsLogFile.is_open())
  {
    this->fpsLogFile << wallTime << "," << fps << std::endl;
    this->fpsLogFile.flush();  // Ensure data is saved immediately
  }
}
