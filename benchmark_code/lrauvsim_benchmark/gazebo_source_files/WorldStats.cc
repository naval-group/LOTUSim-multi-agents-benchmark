...
class WorldStatsPrivate
{
  ...
  /// \brief Output file stream for real time
  public: std::ofstream realUpdateFile;

  /// \brief Output file stream for RTF
  public: std::ofstream rtfFile;

  /// \brief Real Time Updated added for benchmark
  public: std::optional<double> lastRealTimeSec;
  public: std::optional<double> lastSimTimeSec;
...
{
  this->dataPtr->realUpdateFile.open("real_update.csv");
  this->dataPtr->rtfFile.open("rtf.csv");

  // Write CSV headers
  this->dataPtr->realUpdateFile << "SimTime,RealUpdateDuration\n";
  this->dataPtr->rtfFile << "SimTime,RTF\n";
}
...
WorldStats::~WorldStats()
{
  if (this->dataPtr->realUpdateFile.is_open())
    this->dataPtr->realUpdateFile.close();
  if (this->dataPtr->rtfFile.is_open())
    this->dataPtr->rtfFile.close();
}
...
void WorldStats::ProcessMsg()
{
  ...
  const double simTimeFiltered = simTimeCount - (*this->dataPtr->simTimeDelayed);
  const double realTimeFiltered = realTimeCount - (*this->dataPtr->realTimeDelayed);

  if (realTimeFiltered > 0)
  {
    const double rtf = math::precision(simTimeFiltered / realTimeFiltered, 4) * 100;
    this->SetRealTimeFactor(QString::number(rtf, 'f', 2) + " %");
  }
  else
  {
    const double rtf = this->dataPtr->msg.real_time_factor() * 100;
    this->SetRealTimeFactor(QString::number(rtf, 'f', 2) + " %");
  }
  ...
  const double simTimeSec = std::chrono::duration_cast<std::chrono::duration<double>>(
      simTimePoint.time_since_epoch()).count();

  const double realTimeSec = std::chrono::duration_cast<std::chrono::duration<double>>(
      realTimePoint.time_since_epoch()).count();

  // Calculate Real Update Duration
  double realUpdateDuration = 0.0;
  if (this->dataPtr->lastRealTimeSec.has_value())
  {
    realUpdateDuration = realTimeSec - this->dataPtr->lastRealTimeSec.value();
  }

  // Save current realTimeSec for next update
  this->dataPtr->lastRealTimeSec = realTimeSec;

  // Write to real update time CSV
  if (this->dataPtr->realUpdateFile.is_open())
    this->dataPtr->realUpdateFile << simTimeSec << "," << realUpdateDuration << std::endl;

  // Write to RTF CSV
  const double rtfRaw = this->dataPtr->msg.real_time_factor();
  if (this->dataPtr->rtfFile.is_open())
    this->dataPtr->rtfFile << simTimeSec << "," << rtfRaw << std::endl;

  // // Write to FPS CSV if available
  // if (this->dataPtr->msg.has_real_time_update_rate())
  // {
  //   const double fps = this->dataPtr->msg.real_time_update_rate();
  //   if (this->dataPtr->fpsFile.is_open())
  //     this->dataPtr->fpsFile << simTimeSec << "," << fps << std::endl;
  // }
}
