...
namespace gazebo
{
  namespace gui
  {
    ...
    class GZ_GUI_VISIBLE MainWindow : public QMainWindow
    {
      ...
      /// \brief Timer for periodically logging FPS
      private: QTimer *fpsTimer = nullptr;

      /// \brief File stream to write FPS values to CSV
      private: std::ofstream fpsLogFile;

      /// \brief Log the current rendering FPS to CSV
      private slots: void LogFPS();  // Use `slots` macro for Qt signal-slot mechanism
    };
  }
}
#endif
