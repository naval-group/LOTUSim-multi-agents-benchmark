...
)
{
  ...
  if(_sdf->HasElement("default_current"))
  {
    ...
    std::cout << "default_current" << std::endl;
...
//  std::cout << "[HydroPlugin] Water current: " << this->dataPtr->waterCurrent << std::endl;
//  std::cout << "[HydroPlugin] Vehicle velocity (world): " << linearVelocity->Data() << std::endl;
//  std::cout << "[HydroPlugin] Vehicle velocity (local relative to water): " << localLinearVelocity << std::endl;
