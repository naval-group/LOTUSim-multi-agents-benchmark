...
{
  gzmsg << ">>> Hello from World::Load()" << std::endl;
  this->dataPtr->loaded = false;
  ...
  // Advertise RTF publishing topic
  this->dataPtr->rtfPub = this->dataPtr->node->Advertise<gazebo::msgs::Any>("~/benchmark/rtf");

  // Start timing
  this->dataPtr->startWallTime = common::Time::GetWallTime();
  ...
  // Initialize wall time for RTF benchmark
  this->dataPtr->startWallTime = common::Time::GetWallTime();
...
void World::RunLoop()
{
  ...
  this->dataPtr->updateTimeCsv.open("update_times.csv", std::ios::out);
  this->dataPtr->updateTimeCsv << "RealUpdateTime (s)" << std::endl;
  ...
  // Close the update time CSV file
  if (this->dataPtr->updateTimeCsv.is_open())
  {
    this->dataPtr->updateTimeCsv.close();
  }
...
void World::Step()
{
      ...
      this->dataPtr->sleepOffset >= common::Time(updatePeriod))
  {
    ...
    // === Compute and publish Real-Time Factor (RTF) ===============
    common::Time simTime = this->dataPtr->simTime;
    common::Time wallTimeElapsed = common::Time::GetWallTime() - this->dataPtr->startWallTime;

    if (wallTimeElapsed.Double() > 0.0 && this->dataPtr->rtfPub)
    {
      double rtf = simTime.Double() / wallTimeElapsed.Double();

      // Create message and set RTF value
      gazebo::msgs::Any rtfMsg;
      rtfMsg.set_type(gazebo::msgs::Any_ValueType_DOUBLE);
      rtfMsg.set_double_value(rtf);

      // Publish on ~/benchmark/rtf topic
      this->dataPtr->rtfPub->Publish(rtfMsg);

      // Optional: print to terminal for debugging
      //gzmsg << "[RTF] Published: " << rtf << std::endl;
      //=============================================================

    }

      // Start measuring update time ===============================
      common::Time stepStartWallTime = common::Time::GetWallTime();

      this->dataPtr->iterations++;
      this->Update();  // Main simulation step

      common::Time stepEndWallTime = common::Time::GetWallTime();
      common::Time realUpdateDuration = stepEndWallTime - stepStartWallTime;

      // Print the Real Update Time
      //std::cout << "[REAL UPDATE TIME] " << realUpdateDuration.Double() << " s" << std::endl;

      if (this->dataPtr->updateTimeCsv.is_open())
      {
          this->dataPtr->updateTimeCsv << realUpdateDuration.Double() << std::endl;
          this->dataPtr->updateTimeCsv.flush(); // Immediately writes it to disk
      }


      if (!this->dataPtr->updateTimeCsv.is_open())
      {
        gzerr << "Failed to open update_times.csv!" << std::endl;
      }
